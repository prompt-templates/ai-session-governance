...
除跨工具交接外，本模板针对常见 AI 失效模式设有专属防护机制：
...
| 版本 | 变更内容 | 对你的意义 |
|---|---|---|
| **v1.7.0** | 交接 Prompt 首段新增明确指示：接收工具必须先读 `AGENTS.md`，再依 §1 序列执行 | 跨工具交接（如 Codex → Claude Code → Gemini CLI）即使接收工具不会自动加载治理文件，也能可靠地衔接 |
| **v1.6.0** | 安装完成后自动输出 Quick Start 操作区块；`CODEBASE_CONTEXT.md` 生成前先备份，并扩大扫描来源（docs/、yaml、.env） | 安装后无需翻 README 找指令；首次工作阶段的上下文采集更完整 |
| **v1.5.0** | 外部 API 代码安全规则（§0b）— AI 必须获取最新文档才能编写 API 调用代码；测试计划治理（§3d）— 合并前必须定义情景矩阵 | 防止 AI 凭记忆臆测 API 及未经测试即合并，无须增加手动步骤 |
