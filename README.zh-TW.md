...
| 版本 | 變更內容 | 對你的意義 |
|---|---|---|
| **v2.4** | AI 現在會在 PLAN 階段主動質疑自己的假設；工作日誌改用精簡格式（每筆記錄縮小約 60%）；歸檔門檻從 800 行降至 400 行（目標 ≤ 200），基於 "lost in the middle" 研究顯示中段內容準確率下降 30%+ | 計劃品質更高、AI 啟動更快、相同空間容納更多歷史 |
...
截至 2026-04-08 的摘要如下：
- AGENTS/INIT 規則同步：已驗證（163 項回歸測試）
- 多平台指標檔行為：已驗證
- 50+ 工作階段的長期縱向效果：尚未驗證
