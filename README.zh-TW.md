...
除跨工具交接外，本範本針對常見 AI 失效模式設有專屬防護機制：
...
| 版本 | 變更內容 | 對你的意義 |
|---|---|---|
| **v1.7.0** | 交接 Prompt 首段新增明確指示：接收工具必須先讀 `AGENTS.md`，再依 §1 序列執行 | 跨工具交接（如 Codex → Claude Code → Gemini CLI）即使接收工具不會自動載入治理檔案，也能可靠地銜接 |
| **v1.6.0** | 安裝完成後自動輸出 Quick Start 操作區塊；`CODEBASE_CONTEXT.md` 生成前先備份，並擴大掃描來源（docs/、yaml、.env） | 安裝後無須翻 README 找指令；首次工作階段的情境擷取更完整 |
| **v1.5.0** | 外部 API 代碼安全規則（§0b）— AI 必須取得最新文件才能撰寫 API 呼叫代碼；測試計劃治理（§3d）— 合併前必須定義情景矩陣 | 防止 AI 憑記憶臆測 API 及未經測試即合併，無須增加手動步驟 |
...
完成步驟 4 的確認後，AI 會先自動建立備份快照，再進行首次寫入。
