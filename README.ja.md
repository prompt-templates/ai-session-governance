...
| **セッションログ自動メンテナンス** | セッション履歴が数千行に増えてAIの起動コンテキストを圧迫すること — `docs/qa/session_log_maintenance.py` による実行ゲートで、起動時チェック・クローズ時適用を行い、トリガー条件時に `dev/archive/` へ自動アーカイブ（ユーザーの手動リマインド不要） |
...
このテンプレートは「実行可能なゲート」で処理します（ルール記憶依存ではありません）：

- 起動時 / クローズ前チェック：`python docs/qa/session_log_maintenance.py --check --session-log dev/SESSION_LOG.md`
- クローズ書き込み前の適用：`python docs/qa/session_log_maintenance.py --apply --session-log dev/SESSION_LOG.md --archive-dir dev/archive`
- コマンド失敗時は closeout を停止し、失敗内容を報告（サイレントスキップ禁止）
...
|---|---|---|
| **v2.7** | handoff / log 膨張に対する root-fix を、実行可能なマトリクス実験で検証（5パッケージ × 6シナリオ、実トークナイザー計測）。ガバナンスは二段構えに更新：`SESSION_HANDOFF` のコンパクト予算 + §4a 実行ゲート（`session_log_maintenance.py --check/--apply`）。実験では `P3`（handoff compact + auto archive）を最良の低リスク案として採用し、過度圧縮案は no-loss QC 失敗で除外。回帰検証も 222-check スイートに拡張。 | 情報を落とさず定量的に削減：平均 **5921 tokens / session**（o200k）、ストレス条件で最大 **16096**、かつ **6/6 シナリオ QC PASS**。line/date の両トリガーで自動アーカイブが動作し、ユーザーの手動リマインドなしで長期運用の起動効率と引き継ぎ信頼性を維持できる。 |
| **v2.6** | `SESSION_LOG.md` から引き継ぎメモを探す際、以前は「ファイル内で最後に出てくるもの」を使っていたが、ログを整理・アーカイブした後は物理的に最後の位置にある古いものが選ばれることがあった。今は「最新の日付のエントリの中にあるもの」を使う仕様に変更し、どんな順序で並んでいても正しく見つかるようになった。`INIT.md` の10ステップ安全確認手順は、ファイル内に2箇所書かれていて8か所以上で文言がずれ始めていたため、先頭を「下の唯一の定義を参照」する3行のポインターに変更した。セッション開始・終了時に表示されるASCIIアート図の選択ルールも見直し、「前回と同じものを避ける」という跨セッションでは守れないルールを「同じセッション内で開始と終了の図を違うものにする」に変更した。ガバナンス文書を編集するだけの作業が「外部APIを呼ぶコード」と誤認されて `CODEBASE_CONTEXT.md` の事前作成を要求されることがあったため、純粋な文書編集はその条件を発動しないと明確化した。`docs/qa/run_checks.sh` の自動チェックは169項から210項に増え、Session IDの書式・禁止コマンドの網羅性・ファイル名のルール・closeout区切り線の規則など、これまでカバーできていなかった項目が加わった。 | ログを並べ替えたり整理した後でも、引き継ぎメモが正しく読み込まれる。ルールの重複や文言のずれが減り、AI動作の予測可能性がさらに向上する。 |
...
├─ docs/
│  └─ qa/
│     └─ session_log_maintenance.py  # §4a 実行型メンテナンスゲート
└─ dev/
   ├─ SESSION_HANDOFF.md
   ├─ SESSION_LOG.md
...
- `docs/qa/session_log_maintenance.py` - 実行型アーカイブメンテナンスツール（`--check` / `--apply` / `--self-test`）
...
2026-04-19時点の要約：
- AGENTS/INIT 規則整合：検証済み（222 項自動回帰テスト）
- Root-fix token matrix：検証済み（5パッケージ × 6シナリオ、最終採用 `P3_handoff_compact_plus_archive`）
