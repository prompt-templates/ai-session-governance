...
|---|---|---|
| **v2.4** | AIがPLAN段階で自らの前提を検証するように；セッションログがリーン形式に変更（エントリあたり約60%縮小）；アーカイブ閾値を800行から400行に引き下げ（目標≤200）、"lost in the middle"研究に基づく — コンテキスト中間部の正確度30%以上低下を回避 | プラン品質の向上、AI起動の高速化、少ないスペースでより多くの履歴を保持 |
...
2026-04-08時点の要約：
- AGENTS/INIT 規則整合：検証済み（163 項回帰テスト）
