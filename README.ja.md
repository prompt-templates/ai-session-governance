...
Codex、Claude、Gemini の時間単位または週単位のトークン配分を使い切った場合でも、このテンプレートにより、別のAIツールが同じプロジェクト状態をそのまま引き継げます。背景説明を最初からやり直す必要はありません。

- コマンドラインAIツール間での永続的な引き継ぎを実現
- 統一ワークフロー：`PLAN -> READ -> CHANGE -> QC -> PERSIST`
- ルールを増やすだけでなく、ドリフトを抑えるガバナンスを内蔵

**[30秒クイックスタート](#quickstart)** · **[インストール](#install)** · **[クイック操作](#quick-operations)**
...
複数のAIツールを併用する開発では、生成品質よりも引き継ぎの崩壊が先に問題化することが一般的です。

代表的な失敗パターン：
- ツール切り替えのたびに文脈が初期化される
- 修正が修正の上に重なり、運用規則が複雑化する
- README、引き継ぎ文書、ログの整合性が崩れる

このテンプレートは、次の3点を必須化します。
1. すべてのセッションに単一の再入経路を設ける
2. すべての作業を同一ワークフローで実行する
3. セッション終了前に追跡可能な記録を必ず残す

---

## :bookmark_tabs: 内蔵セーフガード

セッション引き継ぎに加え、AI が陥りやすい失敗パターンに対するセーフガードを備えています。
...
|---|---|---|
| **v1.7.0** | 引き継ぎPromptの冒頭に明示的な指示を追加：受信ツールはまず `AGENTS.md` を読み、§1の起動シーケンスに従う | ツール間引き継ぎ（例：Codex → Claude Code → Gemini CLI）が、受信ツールがガバナンスファイルを自動ロードしない場合でも確実に機能する |
| **v1.6.0** | インストール完了後にQuick Startブロックを自動出力；`CODEBASE_CONTEXT.md`の生成前にバックアップを取り、スキャン対象を拡大（docs/、yaml、.env） | インストール後にREADMEを探す必要なし；初回セッションのコンテキスト取得がより完全になる |
| **v1.5.0** | 外部APIコード安全規則（§0b）— AIはAPIコードを書く前に最新ドキュメントを取得する必要がある；テスト計画ガバナンス（§3d）— マージ前にシナリオマトリックスの定義が必須 | 記憶からのAPI推測と未検証マージを防止し、手動ステップを増やさない |
...
手動セットアップは不要です。AIが全体を自動処理し、既存の `AGENTS.md`、`CLAUDE.md`、`GEMINI.md` も安全に統合します。
公開利用の大半では `INIT.md` だけで導入可能です。
このリポジトリ一式を手動で別プロジェクト直下へコピーせず、必ず `INIT.md` を使って導入してください。
...
自然言語で実行できます。以下はそのまま貼り付け可能な定型文です。
...
いいえ。小規模でも即時に効果があり、長期運用では規模が大きいほど効果が拡大します。

### :small_blue_diamond: 2) 初日から `PROJECT_MASTER_SPEC.md` は必要ですか？
必須ではありません。まずは `AGENTS.md` + `SESSION_HANDOFF.md` + `SESSION_LOG.md` で運用可能です。

### :small_blue_diamond: 3) これはコーディング規約ですか？
主目的はコーディング規約ではありません。AIが読む・変更する・検証する・引き継ぐ手順を定義するガバナンスモデルです。

### :small_blue_diamond: 4) セッションは遅くなりますか？
開始時に前処理は増えますが、再説明や重複修正による総工数増加より通常は小さくなります。

### :small_blue_diamond: 5) 既存のREADMEや内部規則は残せますか？
はい。既存資産を置換するのではなく、統合する前提で設計されています。
