...
|---|---|---|
| **v3.0** | ガバナンス文書を大幅にスリム化：AGENTS.md は 734 行から 487 行へ（−33.7%）、ルールはすべて保持。セッション起動時のシステムプロンプト・トークンコストが約 15.6% 減少。Legacy quarantine 機構で 89 件の履歴ドリフト防止チェックを自動チェーンの第二層ハーネスに分離 — メインチェックは軽量化、ただしリリース時の legacy バイパスは禁止のため履歴セーフガードが無音で失われることはない。`dev/SESSION_STATE_DETAIL.md` や `dev/PROJECT_MASTER_SPEC.md` を作成済みのユーザーが再インストールしても、これらが自動でバックアップされる（データ安全なアップグレードパス）。 | システムプロンプト中のガバナンス文が減る → ルール遵守率が向上（業界データ：短いルール約 89% vs 冗長な記述約 35%）；ローカルファイルはアップグレード時に保持；複数 LLM 間で相互運用性あり（Claude Code、Claude Cowork、OpenAI Codex CLI、Gemini CLI、Web LLMs）— フック依存ゼロ。 |
...
2026-04-25（v3.0）時点の要約：
- AGENTS/INIT 規則整合：検証済み（245 項自動回帰 — 156 メイン + 89 legacy 自動チェーン）
- AGENTS.md L4 削減：734 → 487 行（−33.7%）、すべてのルールと 212 個の grep アンカーを 14 ラウンドの検証で完全保持
