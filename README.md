...
|---|---|---|
| **v2.4** | AI now grades task risk at PLAN phase — high-risk tasks (≥3 files, ambiguous scope, destructive ops, external systems) pause for user confirmation before proceeding; session log entries use a lean format (~60% smaller); archive threshold lowered from 800 to 400 lines (target ≤ 200) based on "lost in the middle" research showing 30%+ accuracy drop for mid-context content | Misunderstood tasks caught before code changes; faster AI startup; more session history fits in less space |
